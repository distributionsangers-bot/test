...
export const ParticipantsView = {
    async openParticipantsModal(shiftId, title) {
        toggleLoader(true);
        ...
        list.addEventListener('change', async (e) => {
            if (e.target.classList.contains('presence-check')) {
                ...
                await ParticipantsView.togglePresence(regId, isPresent);
            }
        });
        ...
        list.addEventListener('click', async (e) => {
            ...
            if (btn.dataset.action === 'force-unsubscribe') {
                ...
                await ParticipantsView.forceUnsubscribe(regId, btn.closest('.p-4')); // Pass row element to remove it
            }
        });
    },

    async togglePresence(regId, isPresent) {
        const { error } = await ParticipantsService.updateAttendance(regId, isPresent);
        if (error) {
            showToast("Erreur mise à jour présence", "error");
            // Revert checkbox if needed? For simplicity we trust optimistic UI or user retries
        } else {
            showToast(isPresent ? 'Présence validée' : 'Présence annulée');
        }
    },

    async forceUnsubscribe(regId, rowElement) {
